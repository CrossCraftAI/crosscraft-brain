package storage

import (
	"testing"

	"github.com/CrossCraftAI/crosscraft-brain/server/internal/credtype"
	"github.com/CrossCraftAI/crosscraft-brain/server/internal/schema"
)

func TestDropboxNodeDefinition(t *testing.T) {
	def := DropboxNode()
	if def.Type != "storage.dropbox" {
		t.Fatalf("unexpected type: %s", def.Type)
	}
	if len(def.Params) == 0 {
		t.Fatal("expected params to be defined")
	}
	if def.Execute == nil {
		t.Fatal("expected execute function to be set")
	}
	if len(def.Outputs) != 2 {
		t.Fatalf("expected 2 outputs, got %d", len(def.Outputs))
	}

	opCount := countStorageOperations(def)
	if opCount != 8 {
		t.Fatalf("expected 8 operations, got %d", opCount)
	}
}

func countStorageOperations(def schema.NodeDefinition) int {
	for _, p := range def.Params {
		if p.Name == "operation" {
			return len(p.Options)
		}
	}
	return 0
}

func TestDropboxNodeUsesRegisteredCredentialType(t *testing.T) {
	reg := credtype.Default()
	credType, ok := reg.Get("dropboxApi")
	if !ok {
		t.Fatal("expected dropboxApi credential type to be registered")
	}
	if credType.DisplayName == "" {
		t.Fatal("expected dropboxApi credential type to have a display name")
	}
	if len(credType.Fields) == 0 {
		t.Fatal("expected dropboxApi credential fields to be defined")
	}
}

func TestDropboxExecuteRequiresCredential(t *testing.T) {
	ctx := &schema.ExecContext{
		Params: map[string]any{
			"operation": "list",
			"path":      "",
		},
		Credential: func(paramName string) (map[string]any, error) {
			return nil, nil
		},
	}
	_, err := executeDropbox(ctx)
	if err == nil {
		t.Fatal("expected error when credential is missing")
	}
}

func TestDropboxExecuteRequiresToken(t *testing.T) {
	ctx := &schema.ExecContext{
		Params: map[string]any{
			"operation": "list",
			"path":      "",
		},
		Credential: func(paramName string) (map[string]any, error) {
			return map[string]any{"accessToken": ""}, nil
		},
	}
	_, err := executeDropbox(ctx)
	if err == nil {
		t.Fatal("expected error when access token is empty")
	}
}

func TestStorageNodeCount(t *testing.T) {
	nodes := Nodes()
	if len(nodes) != 2 {
		t.Fatalf("expected 2 storage nodes, got %d", len(nodes))
	}
	types := make(map[string]bool)
	for _, n := range nodes {
		types[n.Type] = true
	}
	for _, want := range []string{"storage.dropbox", "storage.box"} {
		if !types[want] {
			t.Fatalf("expected node type %q in Nodes()", want)
		}
	}
}
